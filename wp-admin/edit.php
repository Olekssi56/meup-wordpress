...
/**
 * Retrieves the WordPress Posts List Table instance.
 *
 * This function is used to get an instance of the `WP_Posts_List_Table` class, which is responsible for rendering the list of posts in the WordPress admin area.
 *
 * @return WP_Posts_List_Table The WordPress Posts List Table instance.
 */
...
/**
 * Retrieves the current action being performed on the posts list table.
 *
 * This function is used to get the current action being performed on the posts list table, such as 'trash', 'delete', or any other custom action defined by the plugin or theme.
 *
 * @return string The current action being performed on the posts list table.
 */
	...
	switch ( $doaction ) {
		...
		case 'print':
			...
			foreach ( (array) $post_ids as $post_id ) {
				...
				$mpdf->AddPage(); // Add a new page for each post
			}

			$upload_dir = wp_upload_dir();
			$pdf_file_path = $upload_dir['basedir'] . '/print_ticket_' . time() . '.pdf';
			$pdf_file_url = $upload_dir['baseurl'] . '/print_ticket_' . time() . '.pdf';
