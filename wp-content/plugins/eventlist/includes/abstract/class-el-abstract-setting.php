...
abstract class EL_Abstract_Setting extends EL_Setting {
		...
		if ( $this->_fields ) {
			...
			if ( $this->_tab ) {
				...
				foreach ( $this->_fields as $id => $groups ) {
					...
					$html[] = '<a href="#'. esc_attr( $id ) .'" id="' . esc_attr( $id ) . '" class="'.esc_attr( $class ).'">' . $groups['title'] . '</a>';
				}
				$html[] = '</h3>';
			}
			...
			if ( $this->_tab ) {
				foreach ( $this->_fields as $id => $groups ) {
					...
					$html[] = '<div data-tab-id="' . $id . '" class="'.esc_attr( $class ).'">';
					$html[] = isset($groups['desc']) ? '<div class="desc_tab">'.$groups['desc'].'</div>' : '';
					$html[] = $this->generate_fields( $groups );
					$html[] = apply_filters( 'el_tab_group_after_content_'.$id, '' );
					$html[] = '</div>';
				}
			} else {
	...
	function generate_fields( $groups = array() ) {
		...
		foreach ( $groups as $key => $group ) {
			...
			if ( $accordion ) {
				$html[] = '<div class="ova_accordion">';
				foreach ( $accordion as $section_key => $section_name ) {
					$html[] = '<h3>'.esc_html( $section_name ).'</h3>';
					$html[] = '<div class="'.esc_attr( $section_key ).'">';
					$html = array_merge( $html, $this->generate_tables_accordion( $group, $section_key ) );
					$html[] = '</div>';
				}
				$html[] = '</div>';
			} else {
				$html = array_merge( $html, $this->generate_tables( $group ) );
			}
		}
